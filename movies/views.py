...
def movies_and_shows(request):
    ...
    for category in categories:
        # Récupérer les films de cette catégorie, triés par popularité
        movies = Movie.objects.filter(categories=category).order_by('-rating', '-created_at')[:8]
        
        if movies.exists():  # Seulement ajouter les catégories qui ont des films
            category_data = {
                'category': category,
                'movies': movies
            }
    ...
    # Si aucune catégorie n'a de films, créer des données par défaut
    if not categories_with_movies:
        # Créer des catégories par défaut pour la démo
        default_categories = [
        ...
        for cat_data in default_categories:
            ...
            categories_with_movies.append({
                'category': category,
                'movies': Movie.objects.none()  # Pas de films pour l'instant
            })
    
    # Récupérer tous les films pour le contexte général
...
def add_to_list(request, slug):
    ...
    if request.method == 'POST':
        try:
            ...
            # Ici vous pouvez implémenter la logique de liste utilisateur
            # Pour l'instant, on simule juste le succès
            return JsonResponse({'success': True, 'message': 'Ajouté à votre liste'})
